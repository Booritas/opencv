...
namespace
{
#define IMGPROC_FLUID [] () { return ncvslideio::compile_args(ncvslideio::gapi::use_only{ncvslideio::gapi::imgproc::fluid::kernels()}); }
}  // anonymous namespace
...
INSTANTIATE_TEST_CASE_P(filter2DTestFluid, Filter2DTest,
                        Combine(Values(CV_8UC1, CV_16UC1, CV_16SC1),
                                Values(ncvslideio::Size(1280, 720),
                                       ncvslideio::Size(128, 128)),
                                Values(-1, CV_32F),
                                Values(IMGPROC_FLUID),
                                Values(ToleranceFilter(1e-4f, 0.01).to_compare_obj()),
                                Values(ncvslideio::Size(3, 3)), // add kernel size=4x4,5x5,7x7 when implementation ready
                                Values(ncvslideio::BORDER_DEFAULT)));
