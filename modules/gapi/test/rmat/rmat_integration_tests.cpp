...
struct RMatIntTestBase {
    ...
    RMatIntTestBase()
        ...
        , comp([](){
              ncvslideio::GMat in;
              auto tmp = ncvslideio::gapi::blur(in, {3,3});
              auto out = ncvslideio::gapi::blur(tmp, {3,3});
              ncvslideio::gapi::island("test", ncvslideio::GIn(in), ncvslideio::GOut(tmp));
              return ncvslideio::GComputation(in, out);
          })
        , inCallbackCalled(false)
        , outCallbackCalled(false) {
        ncvslideio::randu(in_mat, ncvslideio::Scalar::all(127), ncvslideio::Scalar::all(40));
    }
    ...
    RMat createRMat(ncvslideio::Mat& mat, bool& callbackCalled) {
        return {ncvslideio::make_rmat<RMatAdapterT>(mat, callbackCalled)};
    }
};
...
struct RMatIntTest : public RMatIntTestBase<RMatAdapterT>
{
    ...
    void run(const In& in, Out& out, ncvslideio::GCompileArgs&& compile_args) {
        for (int i = 0; i < 2; i++) {
            EXPECT_FALSE(this->inCallbackCalled);
            EXPECT_FALSE(this->outCallbackCalled);
            auto compile_args_copy = compile_args;
            this->comp.apply(ncvslideio::gin(in), ncvslideio::gout(out), std::move(compile_args_copy));
            EXPECT_FALSE(this->inCallbackCalled);
...
{
    template <typename M>
    ncvslideio::GMatDesc getDesc(const M& m) { return ncvslideio::descr_of(m); }

    void checkOutput(const ncvslideio::Mat&) { this->check(); }

    void checkOutput(const RMat& rm) {
        auto view = rm.access(RMat::Access::R);
        this->out_mat = ncvslideio::Mat(view.size(), view.type(), view.ptr());
        this->check();
    }
    ...
    void run(const In& in, Out& out, ncvslideio::GCompileArgs&& compile_args) {
        ...
        sc.setSource(ncvslideio::gin(in));
        sc.start();
        ...
        EXPECT_FALSE(this->inCallbackCalled);
        EXPECT_FALSE(this->outCallbackCalled);
        while (sc.pull(ncvslideio::gout(out)) && frame < num_frames) {
            frame++;
...
struct OcvKernels {
    ncvslideio::GKernelPackage kernels() { return ncvslideio::gapi::imgproc::cpu::kernels(); }
};
struct FluidKernels {
    ncvslideio::GKernelPackage kernels() { return ncvslideio::gapi::imgproc::fluid::kernels(); }
};
...
TYPED_TEST(RMatIntTypedTest, In) {
    ...
    this->run(in_rmat, this->out_mat, ncvslideio::compile_args(this->kernels()));
}

TYPED_TEST(RMatIntTypedTest, Out) {
    auto out_rmat = this->createRMat(this->out_mat, this->outCallbackCalled);
    this->run(this->in_mat, out_rmat, ncvslideio::compile_args(this->kernels()));
}

TYPED_TEST(RMatIntTypedTest, InOut) {
    auto  in_rmat = this->createRMat(this->in_mat, this->inCallbackCalled);
    auto out_rmat = this->createRMat(this->out_mat, this->outCallbackCalled);
    this->run(in_rmat, out_rmat, ncvslideio::compile_args(this->kernels()));
}
