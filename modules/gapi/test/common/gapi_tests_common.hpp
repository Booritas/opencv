...
namespace
{
    inline std::ostream& operator<<(std::ostream& o, const ncvslideio::GCompileArg& arg)
    {
        return o << (arg.tag.empty() ? "empty" : arg.tag);
    }

    inline std::ostream& operator<<(std::ostream& o, const ncvslideio::gapi::wip::draw::Prim& p)
    {
        using namespace ncvslideio::gapi::wip::draw;
        switch (p.index())
        {
            case Prim::index_of<Rect>():
                o << "ncvslideio::gapi::draw::Rect";
                break;
            case Prim::index_of<Text>():
                o << "ncvslideio::gapi::draw::Text";
                break;
            case Prim::index_of<Circle>():
                o << "ncvslideio::gapi::draw::Circle";
                break;
            case Prim::index_of<Line>():
                o << "ncvslideio::gapi::draw::Line";
                break;
            case Prim::index_of<Mosaic>():
                o << "ncvslideio::gapi::draw::Mosaic";
                break;
            case Prim::index_of<Image>():
                o << "ncvslideio::gapi::draw::Image";
                break;
            case Prim::index_of<Poly>():
                o << "ncvslideio::gapi::draw::Poly";
                break;
    ...
    template <typename T> inline void initPointRandU(ncvslideio::RNG &rng, ncvslideio::Point_<T>& pt)
    {
        GAPI_Assert(std::is_integral<T>::value);
        pt = ncvslideio::Point_<T>(static_cast<T>(static_cast<char>(rng(CHAR_MAX + 1U))),
                           static_cast<T>(static_cast<char>(rng(CHAR_MAX + 1U))));
    }

    template <typename T> inline void initPointRandU(ncvslideio::RNG &rng, ncvslideio::Point3_<T>& pt)
    {
        GAPI_Assert(std::is_integral<T>::value);
        pt = ncvslideio::Point3_<T>(static_cast<T>(static_cast<char>(rng(CHAR_MAX + 1U))),
                            static_cast<T>(static_cast<char>(rng(CHAR_MAX + 1U))),
                            static_cast<T>(static_cast<char>(rng(CHAR_MAX + 1U))));
    }

    template <typename F> inline void initFloatPointRandU(ncvslideio::RNG &rng, ncvslideio::Point_<F> &pt)
    {
        GAPI_Assert(std::is_floating_point<F>::value);
        static const int fscale = 256;  // avoid bits near ULP, generate stable test input
        pt = ncvslideio::Point_<F>(rng.uniform(0, 255 * fscale) / static_cast<F>(fscale),
                           rng.uniform(0, 255 * fscale) / static_cast<F>(fscale));
    }

    template<> inline void initPointRandU(ncvslideio::RNG &rng, ncvslideio::Point2f &pt)
    { initFloatPointRandU(rng, pt); }

    template<> inline void initPointRandU(ncvslideio::RNG &rng, ncvslideio::Point2d &pt)
    { initFloatPointRandU(rng, pt); }

    template <typename F> inline void initFloatPointRandU(ncvslideio::RNG &rng, ncvslideio::Point3_<F> &pt)
    {
        GAPI_Assert(std::is_floating_point<F>::value);
        static const int fscale = 256;  // avoid bits near ULP, generate stable test input
        pt = ncvslideio::Point3_<F>(rng.uniform(0, 255 * fscale) / static_cast<F>(fscale),
                            rng.uniform(0, 255 * fscale) / static_cast<F>(fscale),
                            rng.uniform(0, 255 * fscale) / static_cast<F>(fscale));
    }

    template<> inline void initPointRandU(ncvslideio::RNG &rng, ncvslideio::Point3f &pt)
    { initFloatPointRandU(rng, pt); }

    template<> inline void initPointRandU(ncvslideio::RNG &rng, ncvslideio::Point3d &pt)
    { initFloatPointRandU(rng, pt); }
} // namespace
...
{
public:
    ncvslideio::Mat in_mat1;
    ncvslideio::Mat in_mat2;
    ncvslideio::Mat out_mat_gapi;
    ncvslideio::Mat out_mat_ocv;

    ncvslideio::Scalar sc;

    // integral Scalar initialization
    ncvslideio::Scalar initScalarRandU(unsigned upper)
    {
        ncvslideio::RNG rng(time(nullptr));
        double s1 = rng(upper);
        double s2 = rng(upper);
        double s3 = rng(upper);
        double s4 = rng(upper);
        return ncvslideio::Scalar(s1, s2, s3, s4);
    }

    // floating-point Scalar initialization (ncvslideio::core)
    ncvslideio::Scalar initScalarRandU()
    {
        ncvslideio::RNG rng(time(nullptr));
    ...
    void initOutMats(ncvslideio::Size sz_in, int dtype)
    {
        if (dtype != -1)
        {
            out_mat_gapi = ncvslideio::Mat(sz_in, dtype);
            out_mat_ocv = ncvslideio::Mat(sz_in, dtype);
        }
    }

    void initMatsRandU(int type, ncvslideio::Size sz_in, int dtype, bool createOutputMatrices = true)
    {
        in_mat1 = ncvslideio::Mat(sz_in, type);
        in_mat2 = ncvslideio::Mat(sz_in, type);
    ...
    void initMatrixRandU(int type, ncvslideio::Size sz_in, int dtype, bool createOutputMatrices = true)
    {
        in_mat1 = ncvslideio::Mat(sz_in, type);
    ...
    void initMatrixRandN(int type, ncvslideio::Size sz_in, int dtype, bool createOutputMatrices = true)
    {
        in_mat1 = ncvslideio::Mat(sz_in, type);
        ncvslideio::randn(in_mat1, ncvslideio::Scalar::all(127), ncvslideio::Scalar::all(40.f));
    ...
    void initMatFromImage(int type, const std::string& fileName)
    {
        ...
        const int readFlags = (channels == 1) ? ncvslideio::IMREAD_GRAYSCALE : ncvslideio::IMREAD_COLOR;
        ncvslideio::Mat mat = ncvslideio::imread(findDataFile(fileName), readFlags);
        if (channels == 4)
        {
            ncvslideio::cvtColor(mat, in_mat1, ncvslideio::COLOR_BGR2BGRA);
        }
        else
        {
            in_mat1 = mat;
    ...
    {
        GAPI_Assert(channels == 1 || channels == 3 || channels == 4);
        const int flags = (channels == 1) ? ncvslideio::IMREAD_GRAYSCALE : ncvslideio::IMREAD_COLOR;

        ncvslideio::Mat m1 = ncvslideio::imread(findDataFile(ncvslideio::format(pattern.c_str(), imgNum)), flags);
        ncvslideio::Mat m2 = ncvslideio::imread(findDataFile(ncvslideio::format(pattern.c_str(), imgNum + 1)), flags);
        if (channels == 4)
        {
            cvtColor(m1, in_mat1, ncvslideio::COLOR_BGR2BGRA);
            cvtColor(m2, in_mat2, ncvslideio::COLOR_BGR2BGRA);
        }
        else
        {
    ...
    template <typename T>
    inline void initPointRandU(ncvslideio::RNG& rng, T& pt) const
...
#endif
    // initialize std::vector<ncvslideio::Point_<T>>/std::vector<ncvslideio::Point3_<T>>
...
#pragma warning(pop)
    ...
    inline void initMatByPointsVectorRandU(const ncvslideio::Size &sz_in)
    {
            std::vector<Pt> in_vector;
            initPointsVectorRandU(sz_in.width, in_vector);
            in_mat1 = ncvslideio::Mat(in_vector, true);
    }

    // initialize Mat by a vector of Points
    template<template <typename> class Pt>
    inline void initMatByPointsVectorRandU(int type, ncvslideio::Size sz_in, int)
    {
        int depth = CV_MAT_DEPTH(type);
        switch (depth)
        {
        ...
        case CV_64F:
            ...
            break;
        case CV_16F:
            initMatByPointsVectorRandU<Pt<ncvslideio::hfloat>>(sz_in);
            break;
        default:
    ...
    void initNothing(int, ncvslideio::Size, int, bool = true) {}
};
...
struct Params : public ParamsBase<std::tuple<MatType2,ncvslideio::Size,MatType2,ncvslideio::GCompileArgs(*)()>,
...
template<typename ...SpecificParams>
struct ParamsSpecific : public ParamsBase<std::tuple<ncvslideio::GCompileArgs(*)()>,
                                          std::tuple<SpecificParams...>>
{
...
struct TestWithParamsBase : TestFunctional, TestWithParam<typename AllParams::params_t>
{
    ...
    inline ncvslideio::GCompileArgs getCompileArgs() const
    {
...
struct TestWithParams : public TestWithParamsBase<Params<SpecificParams...>>
{
    ...
    ncvslideio::Size sz    = this->template getCommonParam<1>();
...
using compare_f = std::function<bool(const ncvslideio::Mat &a, const ncvslideio::Mat &b)>;
using compare_scalar_f = std::function<bool(const ncvslideio::Scalar &a, const ncvslideio::Scalar &b)>;
using compare_rect_f = std::function<bool(const ncvslideio::Rect &a, const ncvslideio::Rect &b)>;
...
using compare_vec_f = std::function<bool(const ncvslideio::Vec<Elem, cn> &a, const ncvslideio::Vec<Elem, cn> &b)>;
...
using CompareMats = CompareF<ncvslideio::Mat, ncvslideio::Mat>;
using CompareScalars = CompareF<ncvslideio::Scalar, ncvslideio::Scalar>;
using CompareRects = CompareF<ncvslideio::Rect, ncvslideio::Rect>;

template<typename Elem>
using CompareVectors = CompareF<std::vector<Elem>, std::vector<Elem>>;

template<typename Elem, int cn>
using CompareVecs = CompareF<ncvslideio::Vec<Elem, cn>, ncvslideio::Vec<Elem, cn>>;
...
struct Wrappable
{
    compare_f to_compare_f()
    {
        ...
        return [t](const ncvslideio::Mat &a, const ncvslideio::Mat &b)
        {
            return t(a, b);
        };
    }
...
struct WrappableScalar
{
    ...
    {
        T t = *static_cast<T*const>(this);
        return [t](const ncvslideio::Scalar &a, const ncvslideio::Scalar &b)
        {
            return t(a, b);
        };
    }
...
struct WrappableRect
{
    ...
    {
        T t = *static_cast<T*const>(this);
        return [t](const ncvslideio::Rect &a, const ncvslideio::Rect &b)
        {
            return t(a, b);
        };
    }
...
struct WrappableVec
{
    ...
    {
        T t = *static_cast<T* const>(this);
        return [t](const ncvslideio::Vec<Elem, cn> &a, const ncvslideio::Vec<Elem, cn> &b)
        {
            return t(a, b);
        };
    }
...
public:
    AbsExact() {}
    bool operator() (const ncvslideio::Mat& in1, const ncvslideio::Mat& in2) const
    {
        if (ncvslideio::norm(in1, in2, NORM_INF) != 0)
        {
...
public:
    ...
    bool operator() (const ncvslideio::Mat& in1, const ncvslideio::Mat& in2) const
    {
        if (ncvslideio::norm(in1, in2, NORM_INF) > _tol)
        {
...
public:
    ...
    bool operator() (const ncvslideio::Mat& in1, const ncvslideio::Mat& in2) const
    {
        ...
        {
            double err = depth >= CV_32F ? ncvslideio::norm(in1, in2, NORM_L1 | NORM_RELATIVE)
                                                     : ncvslideio::norm(in1, in2, NORM_INF);
            double tolerance = depth >= CV_32F ? _tol : _tol8u;
            if (err > tolerance)
            {
                std::cout << "Tolerance_FloatRel_IntAbs error: err=" << err
                          << "  tolerance=" << tolerance
                          << "  depth=" << ncvslideio::typeToString(depth) << std::endl;
                return false;
            }
            else
            {
...
public:
    ...
    {
        Mat diff;
        ncvslideio::absdiff(in1, in2, diff);
        Mat err_mask = diff > _tol;
        int err_points = (ncvslideio::countNonZero)(err_mask.reshape(1));
...
public:
    ...
    bool operator() (const ncvslideio::Mat& in1, const ncvslideio::Mat& in2) const
    {
        int depth = CV_MAT_DEPTH(in1.type());
        {
            double err_Inf = ncvslideio::norm(in1, in2, NORM_INF);
            if (err_Inf > _inf_tol)
            {
                std::cout << "ToleranceFilter error: err_Inf=" << err_Inf << "  tolerance=" << _inf_tol << std::endl;
                return false;
            }
            double err = ncvslideio::norm(in1, in2, NORM_L2 | NORM_RELATIVE);
            double tolerance = depth >= CV_32F ? _tol : _tol8u;
            if (err > tolerance)
            {
                std::cout << "ToleranceFilter error: err=" << err << "  tolerance=" << tolerance
                          << "  depth=" << ncvslideio::depthToString(depth)
                          << std::endl;
                return false;
            }
        }
    ...
    {
        {
            ...
            double err = ncvslideio::norm(in1, in2, NORM_L1 | NORM_RELATIVE);
            if (err > _tol)
            {
...
public:
    ...
    bool operator() (const ncvslideio::Scalar& in1, const ncvslideio::Scalar& in2) const
    {
...
public:
    ...
    bool operator() (const ncvslideio::Rect& in1, const ncvslideio::Rect& in2) const
    {
...
public:
    ...
    {
        if (ncvslideio::norm(in1, in2, NORM_INF, ncvslideio::noArray()) != 0)
        {
...
public:
    ...
    bool operator() (const ncvslideio::Vec<Elem, cn> &in1, const ncvslideio::Vec<Elem, cn> &in2) const
    {
        double abs_err  = ncvslideio::norm(in1, in2, ncvslideio::NORM_L1);
        double in2_norm = ncvslideio::norm(in2, ncvslideio::NORM_L1);
        // Checks to avoid dividing by zero
        double err = abs_err ? abs_err / (in2_norm ? in2_norm : ncvslideio::norm(in1, ncvslideio::NORM_L1))
                             : abs_err;
        if (err > _tol)
        {
