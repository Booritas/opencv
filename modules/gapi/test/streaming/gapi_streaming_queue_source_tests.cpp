...
TEST(GAPI_Streaming_Queue_Source, SmokeTest) {
    ...
    ncvslideio::GMat in;
    ncvslideio::GMat out = in + 1;
    ncvslideio::GStreamingCompiled comp = ncvslideio::GComputation(in, out).compileStreaming();

    // Queue source needs to know format information to maintain contracts
    auto src = std::make_shared<ncvslideio::gapi::wip::QueueSource<ncvslideio::Mat> >
        (ncvslideio::GMatDesc{CV_8U, 1, ncvslideio::Size{128, 128}});

    comp.setSource(ncvslideio::gin(src->ptr()));
    comp.start();
    ...
    ncvslideio::Mat eye = ncvslideio::Mat::eye(ncvslideio::Size{128, 128}, CV_8UC1);
    ...
    ASSERT_TRUE(comp.pull(ncvslideio::gout(result)));
...
TEST(GAPI_Streaming_Queue_Source, Mixed) {
    ...
    ncvslideio::GMat in1;
    ncvslideio::GMat in2;
    ncvslideio::GMat out = in2 - in1;
    ncvslideio::GStreamingCompiled comp = ncvslideio::GComputation(in1, in2, out).compileStreaming();

    // Queue source needs to know format information to maintain contracts
    auto src1 = std::make_shared<ncvslideio::gapi::wip::QueueSource<ncvslideio::Mat> >
        (ncvslideio::GMatDesc{CV_8U, 3, ncvslideio::Size{768, 576}});

    std::shared_ptr<ncvslideio::gapi::wip::IStreamSource> src2;
    auto path = findDataFile("ncvslideio/video/768x576.avi");
    try {
        src2 = ncvslideio::gapi::wip::make_src<ncvslideio::gapi::wip::GCaptureSource>(path);
    } catch(...) {
        throw SkipTestException("Video file can not be opened");
    }

    comp.setSource(ncvslideio::gin(src1->ptr(), src2)); // FIXME: quite inconsistent
    comp.start();

    ncvslideio::Mat eye = ncvslideio::Mat::eye(ncvslideio::Size{768, 576}, CV_8UC3);
    src1->push(eye);    // Push I (identity matrix)
    src1->push(eye);    // Push I (again)

    ncvslideio::Mat ref, result;
    ncvslideio::VideoCapture cap(path);
...
TEST(GAPI_Streaming_Queue_Input, SmokeTest) {
    ...
    ncvslideio::GStreamingCompiled comp = ncvslideio::GComputation(ncvslideio::GIn(in1, in2), ncvslideio::GOut(out))
        .compileStreaming();

    // FIXME: This API is too raw
    ncvslideio::gapi::wip::QueueInput input({
            ncvslideio::GMetaArg{ ncvslideio::GMatDesc{CV_8U, 1, ncvslideio::Size{64,64} } },
            ncvslideio::GMetaArg{ ncvslideio::empty_scalar_desc() }
        });
    comp.setSource(input); // Implicit conversion allows it to be passed as-is.
    comp.start();

    // Push data via queue input
    ncvslideio::Mat eye = ncvslideio::Mat::eye(ncvslideio::Size{64, 64}, CV_8UC1);
    input.push(ncvslideio::gin(eye, ncvslideio::Scalar(1)));
    input.push(ncvslideio::gin(eye, ncvslideio::Scalar(2)));
    input.push(ncvslideio::gin(eye, ncvslideio::Scalar(3)));
