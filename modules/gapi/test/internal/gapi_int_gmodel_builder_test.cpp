...
namespace
{
    namespace D = ncvslideio::detail;
    ncvslideio::GMat unaryOp(ncvslideio::GMat m)
    {
        return ncvslideio::GCall(ncvslideio::GKernel{ "gapi.test.unaryop"
                                    , ""
                                    , nullptr
                                    , { GShape::GMAT }
                                    , { D::OpaqueKind::CV_UNKNOWN }
                                    , { D::HostCtor{ncvslideio::util::monostate{}} }
                                    , { D::OpaqueKind::CV_UNKNOWN }
                                    }).pass(m).yield(0);
    }

    ncvslideio::GMat binaryOp(ncvslideio::GMat m1, ncvslideio::GMat m2)
    {
        return ncvslideio::GCall(ncvslideio::GKernel{ "gapi.test.binaryOp"
                                    , ""
                                    , nullptr
                                    , { GShape::GMAT }
    ...
    std::vector<ade::NodeHandle> collectOperations(const ncvslideio::gimpl::GModel::Graph& gr)
    {
        std::vector<ade::NodeHandle> ops;
        for (const auto& nh : gr.nodes())
        {
            if (gr.metadata(nh).get<ncvslideio::gimpl::NodeType>().t == ncvslideio::gimpl::NodeType::OP)
                ops.push_back(nh);
        }
        return ops;
    }

    ade::NodeHandle inputOf(ncvslideio::gimpl::GModel::Graph& gm, ade::NodeHandle nh, std::size_t port)
    {
        for (const auto& eh : nh->inEdges())
        {
            if (gm.metadata(eh).get<ncvslideio::gimpl::Input>().port == port)
            {
                return eh->srcNode();
            }
        }
...
{
    ncvslideio::GMat in;
    ncvslideio::GMat out = test::unaryOp(in);

    auto unrolled = ncvslideio::gimpl::unrollExpr(ncvslideio::GIn(in).m_args, ncvslideio::GOut(out).m_args);
...
{
    ncvslideio::GMat in;
    ncvslideio::GMat out = test::unaryOp(test::unaryOp(in));
...
{
    ncvslideio::GMat in1, in2;                                      // in1 -> unaryOp() -> u_op1 -> unaryOp() -> out
    auto u_op1 = test::unaryOp(in1);                        // in2 -> unaryOp() -> u_op2
    auto u_op2 = test::unaryOp(in2);
    auto out   = test::unaryOp(u_op1);

    EXPECT_THROW(ncvslideio::gimpl::unrollExpr(ncvslideio::GIn(in1, in2).m_args, ncvslideio::GOut(out).m_args), std::logic_error);
}

TEST(GModelBuilder, Unroll_Parallel_Path)
{
    ncvslideio::GMat in1, in2;                                      // in1 -> unaryOp() -> out1
    auto out1 = test::unaryOp(in1);                         // in2 -> unaryOp() -> out2
    auto out2 = test::unaryOp(in2);

    auto unrolled = ncvslideio::gimpl::unrollExpr(ncvslideio::GIn(in1, in2).m_args, ncvslideio::GOut(out1, out2).m_args);
...
TEST(GModelBuilder, Unroll_WithBranch)
{
    ...
    auto unrolled = ncvslideio::gimpl::unrollExpr(ncvslideio::GIn(in).m_args, ncvslideio::GOut(out1, out2).m_args);
...
TEST(GModelBuilder, Build_Unary)
{
    ...
    ncvslideio::gimpl::GModelBuilder(g).put(ncvslideio::GIn(in).m_args, ncvslideio::GOut(out).m_args);
...
TEST(GModelBuilder, Constant_GScalar)
{
    ...
    ncvslideio::GMat in;
    ncvslideio::GScalar c_s = 5;
    auto out = test::unaryOp((in + 3) * c_s);    // 3 converted to GScalar

    ade::Graph g;
    ncvslideio::gimpl::GModel::Graph gm(g);
    ncvslideio::gimpl::GModel::init(gm);
    auto proto_slots = ncvslideio::gimpl::GModelBuilder(g).put(ncvslideio::GIn(in).m_args, ncvslideio::GOut(out).m_args);
    ...
    ASSERT_TRUE(gm.metadata(addC_nh).get<ncvslideio::gimpl::NodeType>().t == ncvslideio::gimpl::NodeType::OP);
    ASSERT_TRUE(gm.metadata(mulC_nh).get<ncvslideio::gimpl::NodeType>().t == ncvslideio::gimpl::NodeType::OP);
    ...
    EXPECT_EQ(3, (util::get<ncvslideio::Scalar>(gm.metadata(s_3).get<ncvslideio::gimpl::ConstValue>().arg))[0]);
    EXPECT_EQ(5, (util::get<ncvslideio::Scalar>(gm.metadata(s_5).get<ncvslideio::gimpl::ConstValue>().arg))[0]);
}
...
TEST(GModelBuilder, Check_Multiple_Outputs)
{
    ...
    ncvslideio::GMat in, r, g, b, i_out1, i_out2, u_out;
    std::tie(r, g, b) = ncvslideio::gapi::split3(in);
    std::tie(i_out1, i_out2) = ncvslideio::gapi::integral(g, 1, 1);
    u_out = test::unaryOp(b);

    ade::Graph gr;
    ncvslideio::gimpl::GModel::Graph gm(gr);
    ncvslideio::gimpl::GModel::init(gm);
    auto proto_slots = ncvslideio::gimpl::GModelBuilder(gr).put(ncvslideio::GIn(in).m_args, ncvslideio::GOut(r, i_out1, i_out2, u_out).m_args);
    ncvslideio::gimpl::Protocol p;
    std::tie(p.inputs, p.outputs, p.in_nhs, p.out_nhs) = proto_slots;

    EXPECT_EQ(4u, static_cast<std::size_t>(p.out_nhs.size()));
    EXPECT_EQ(0u, gm.metadata(p.out_nhs[0]->inEdges().front()).get<ncvslideio::gimpl::Output>().port);
    EXPECT_EQ(0u, gm.metadata(p.out_nhs[1]->inEdges().front()).get<ncvslideio::gimpl::Output>().port);
    EXPECT_EQ(1u, gm.metadata(p.out_nhs[2]->inEdges().front()).get<ncvslideio::gimpl::Output>().port);
    EXPECT_EQ(0u, gm.metadata(p.out_nhs[3]->inEdges().front()).get<ncvslideio::gimpl::Output>().port);
    for (const auto it : ade::util::indexed(p.out_nhs))
    {
        const auto& out_nh = ade::util::value(it);

        EXPECT_EQ(ncvslideio::gimpl::NodeType::DATA, gm.metadata(out_nh).get<ncvslideio::gimpl::NodeType>().t);
        EXPECT_EQ(GShape::GMAT, gm.metadata(out_nh).get<ncvslideio::gimpl::Data>().shape);
    }
}
...
{
    ncvslideio::GMat in;
    auto yuv_p = ncvslideio::gapi::split3(in);

    ade::Graph g;
    ncvslideio::gimpl::GModel::Graph gm(g);
    ncvslideio::gimpl::GModel::init(gm);
    ncvslideio::gimpl::GModelBuilder(g).put(ncvslideio::GIn(in).m_args, ncvslideio::GOut(std::get<0>(yuv_p)).m_args);
...
{
    ncvslideio::GMat in, y, u, v;
    std::tie(y, u, v) = ncvslideio::gapi::split3(in);
    auto y_blur = ncvslideio::gapi::gaussianBlur(y, ncvslideio::Size(3, 3), 1);

    ade::Graph g;
    ncvslideio::gimpl::GModel::Graph gm(g);
    ncvslideio::gimpl::GModel::init(gm);
    ncvslideio::gimpl::GModelBuilder(g).put(ncvslideio::GIn(in).m_args, ncvslideio::GOut(y_blur).m_args);
...
TEST(GModelBuilder, Add_Nodes_To_Unused_Nodes)
{
    ...
    auto y_blur = ncvslideio::gapi::gaussianBlur(y, ncvslideio::Size(3, 3), 1);
    // unused nodes
    auto u_blur = ncvslideio::gapi::gaussianBlur(y, ncvslideio::Size(3, 3), 1);
    auto v_blur = ncvslideio::gapi::gaussianBlur(y, ncvslideio::Size(3, 3), 1);
...
TEST(GModelBuilder, Unlisted_Inputs)
{
    ...
    EXPECT_THROW(ncvslideio::gimpl::GModelBuilder(g).put(ncvslideio::GIn(in1).m_args, ncvslideio::GOut(out).m_args), std::logic_error);
}
...
TEST(GModelBuilder, Unroll_No_Link_Between_In_And_Out)
{
    ...
    ncvslideio::GMat in, other;
    auto u_op = test::unaryOp(in);
    auto out  = test::unaryOp(other);

    EXPECT_THROW(ncvslideio::gimpl::unrollExpr(ncvslideio::GIn(in).m_args, ncvslideio::GOut(out).m_args), std::logic_error);
}
...
TEST(GModel_builder, Check_Binary_Op)
{
    ...
    ncvslideio::gimpl::GModel::Graph gm(g);
    ncvslideio::gimpl::GModel::init(gm);
    auto proto_slots = ncvslideio::gimpl::GModelBuilder(g).put(ncvslideio::GIn(in1, in2).m_args, ncvslideio::GOut(out).m_args);
    ...
    EXPECT_EQ("gapi.test.binaryOp", gm.metadata(ops.front()).get<ncvslideio::gimpl::Op>().k.name);
...
TEST(GModelBuilder, Add_Operation_With_Two_Out_One_Time)
{
    ...
    ncvslideio::GMat in, out_b1, out_b2;
    std::tie(out_b1, out_b2) = ncvslideio::gapi::integral(in, 1, 1);
    ...
    ncvslideio::gimpl::GModel::Graph gm(g);
    ncvslideio::gimpl::GModel::init(gm);
    auto proto_slots = ncvslideio::gimpl::GModelBuilder(g).put(ncvslideio::GIn(in).m_args, ncvslideio::GOut(out1, out2).m_args);
    ...
    EXPECT_EQ("org.opencv.core.matrixop.integral", gm.metadata(integral_nh).get<ncvslideio::gimpl::Op>().k.name);
...
TEST(GModelBuilder, Add_Operation_With_One_Out_One_Time)
{
    ...
    ncvslideio::gimpl::GModel::Graph gm(g);
    ncvslideio::gimpl::GModel::init(gm);
    auto proto_slots = ncvslideio::gimpl::GModelBuilder(g).put(ncvslideio::GIn(in1, in2).m_args, ncvslideio::GOut(out1, out2).m_args);
    ncvslideio::gimpl::Protocol p;
    std::tie(p.inputs, p.outputs, p.in_nhs, p.out_nhs) = proto_slots;
