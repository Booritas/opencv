...
struct Unrolled
{
    std::vector<ncvslideio::GNode> all_ops;
    GOriginSet             all_data;
...
class GAPI_EXPORTS GModelBuilder
{
    ...
    std::unordered_map<const ncvslideio::GNode::Priv*, ade::NodeHandle> m_graph_ops;
    GOriginMap<ade::NodeHandle> m_graph_data;

    // Internal methods for mapping APIs into ADE during put()
    ade::NodeHandle put_OpNode(const ncvslideio::GNode &node);
    ade::NodeHandle put_DataNode(const ncvslideio::GOrigin &origin);
...
protected:
    ade::NodeHandle opNode(ncvslideio::GMat gmat);
};
