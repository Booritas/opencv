...
namespace detail
{
    ...
    template<> struct ProtoToParam<ncvslideio::GMat>    { using type = ncvslideio::Mat; };
    template<> struct ProtoToParam<ncvslideio::GScalar> { using type = ncvslideio::Scalar; };
    template<typename U> struct ProtoToParam<ncvslideio::GArray<U> >  { using type = std::vector<U>; };
    template<> struct ProtoToParam<ncvslideio::GArray<ncvslideio::GMat>>      { using type = std::vector<ncvslideio::Mat>; };
    template<typename U> struct ProtoToParam<ncvslideio::GOpaque<U> > { using type = U; };
    template<typename T> using ProtoToParamT = typename ProtoToParam<T>::type;

    template<typename T> struct ProtoToMeta;
    template<> struct ProtoToMeta<ncvslideio::GMat>     { using type = ncvslideio::GMatDesc; };
    template<> struct ProtoToMeta<ncvslideio::GScalar>  { using type = ncvslideio::GScalarDesc; };
    template<typename U> struct ProtoToMeta<ncvslideio::GArray<U> >  { using type = ncvslideio::GArrayDesc; };
    template<typename U> struct ProtoToMeta<ncvslideio::GOpaque<U> > { using type = ncvslideio::GOpaqueDesc; };
 ...
 *  typed.
 *
 *  There is no need to use ncvslideio::gin() or ncvslideio::gout() modifiers with
 ...
 *  Here:
 *
 *  - ncvslideio::GComputation object is created with a lambda constructor
 ...
 *    here. C++ compiler wouldn't notice that since the ncvslideio::GComputation
...
public:
    ...
    private:
        ...
        ncvslideio::GCompiled m_comp;

        explicit GCompiledT(const ncvslideio::GCompiled &comp) : m_comp(comp) {}
    ...
    public:
        ...
        {
            m_comp(ncvslideio::gin(inArgs...), ncvslideio::gout(outArg));
        }
...
private:
    ...
    {
        return Captured(g(args...), ncvslideio::GIn(args...));
    }
...
public:
    GComputationT(const Gen &generator)
        ...
        , m_comp(ncvslideio::GProtoInputArgs(std::move(m_capture.second)),
                 ncvslideio::GOut(m_capture.first))
    {
    }
    ...
    void apply(detail::ProtoToParamT<Args>... inArgs,
               ...
               GCompileArgs &&args)
    {
        m_comp.apply(ncvslideio::gin(inArgs...), ncvslideio::gout(outArg), std::move(args));
    }
...
public:
    ...
    public:
        ...
        {
            m_comp(ncvslideio::gin(inArgs...), ncvslideio::gout(outArgs...));
        }
...
private:
    ...
    {
        return Captured(ncvslideio::GOut(std::get<IIs>(rr)...).m_args, args);
    }

    Captured capture(const Gen& g, Args... args)
    {
        return capture(ncvslideio::GIn(args...).m_args, g(args...), typename detail::MkSeq<sizeof...(R)>::type());
    }
...
public:
    GComputationT(const Gen &generator)
        : m_capture(capture(generator, detail::make_default<Args>()...))
        , m_comp(ncvslideio::GProtoInputArgs(std::move(m_capture.second)),
                 ncvslideio::GProtoOutputArgs(std::move(m_capture.first)))
    {
    }
    ...
    void apply(detail::ProtoToParamT<Args>... inArgs,
               ...
               GCompileArgs &&args)
    {
        m_comp.apply(ncvslideio::gin(inArgs...), ncvslideio::gout(outArgs...), std::move(args));
    }
