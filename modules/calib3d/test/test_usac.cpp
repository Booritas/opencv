...
*/
static int generatePoints (ncvslideio::RNG &rng, ncvslideio::Mat &pts1, ncvslideio::Mat &pts2, ncvslideio::Mat &K1, ncvslideio::Mat &K2,
    ...
    auto eulerAnglesToRotationMatrix = [] (double pitch, double yaw, double roll) {
        ...
        ncvslideio::Matx33d R_x (1, 0, 0, 0, cos(roll), -sin(roll), 0, sin(roll), cos(roll));
        // Calculate rotation about y axis
        ncvslideio::Matx33d R_y (cos(pitch), 0, sin(pitch), 0, 1, 0, -sin(pitch), 0, cos(pitch));
        // Calculate rotation about z axis
        ncvslideio::Matx33d R_z (cos(yaw), -sin(yaw), 0, sin(yaw), cos(yaw), 0, 0, 0, 1);
        return ncvslideio::Mat(R_z * R_y * R_x); // Combined rotation matrix
    };
    ...
    ncvslideio::Mat R = eulerAnglesToRotationMatrix(rng.uniform(pitch_min, pitch_max),
    ...
    ncvslideio::Vec3d t (rng.uniform(-0.5f, 0.5f), rng.uniform(-0.5f, 0.5f), rng.uniform(1.0f, 2.0f));

    // generate random calibration
    auto getRandomCalib = [&] () {
        return ncvslideio::Mat(ncvslideio::Matx33d(rng.uniform(100.0, 1000.0), 0, rng.uniform(100.0, 100.0),
    ...
    auto updateTranslation = [] (const ncvslideio::Mat &pts, const ncvslideio::Mat &R_, ncvslideio::Vec3d &t_) {
        // Make sure the shape is in front of the camera
        ncvslideio::Mat points3d_transformed = R_ * pts + t_ * ncvslideio::Mat::ones(1, pts.cols, pts.type());
        double min_dist, max_dist;
        ncvslideio::minMaxIdx(points3d_transformed.row(2), &min_dist, &max_dist);
    ...
    ncvslideio::Mat points3d;
    if (test_case == TestSolver::Homogr) {
        points3d.create(2, inl_size, pts_type);
        rng.fill(points3d, ncvslideio::RNG::UNIFORM, 0.0, 1.0); // keep small range
        // inliers must be planar points, let their 3D coordinate be 1
        ncvslideio::vconcat(points3d, ncvslideio::Mat::ones(1, inl_size, points3d.type()), points3d);
        ...
        rng.fill(points3d, ncvslideio::RNG::UNIFORM, 0.0, 1.0);
    } else if (test_case == TestSolver::PnP) {
        //pts1 are image points, pts2 are object points
        pts2.create(3, inl_size, pts_type); // 3D inliers
        rng.fill(pts2, ncvslideio::RNG::UNIFORM, 0, 1);

        updateTranslation(pts2, R, t);

        // project 3D points (pts2) on image plane (pts1)
        pts1 = K1 * (R * pts2 + t * ncvslideio::Mat::ones(1, pts2.cols, pts2.type()));
        ncvslideio::divide(pts1.row(0), pts1.row(2), pts1.row(0));
        ncvslideio::divide(pts1.row(1), pts1.row(2), pts1.row(1));
        // make 2D points
        pts1 = pts1.rowRange(0, 2);

        // create random outliers
        ncvslideio::Mat pts_outliers = ncvslideio::Mat(5, out_size, pts2.type());
        rng.fill(pts_outliers, ncvslideio::RNG::UNIFORM, 0, 1000);

        // merge inliers with random image points = outliers
        ncvslideio::hconcat(pts1, pts_outliers.rowRange(0, 2), pts1);
        // merge 3D inliers with 3D outliers
        ncvslideio::hconcat(pts2, pts_outliers.rowRange(2, 5), pts2);

        // add Gaussian noise to image points
        ncvslideio::Mat noise(pts1.rows, pts1.cols, pts1.type());
        rng.fill(noise, ncvslideio::RNG::NORMAL, 0, noise_std);
        pts1 += noise;
        return inl_size;
    } else if (test_case == TestSolver::Affine) {
    } else
        CV_Error(ncvslideio::Error::StsBadArg, "Unknown solver!");
    ...
    if (test_case != TestSolver::PnP) {
        ...
        if (test_case != TestSolver::Affine) {
            ...
            pts1 = K1 * points3d;
            pts2 = K2 * (R * points3d + t * ncvslideio::Mat::ones(1, points3d.cols, points3d.type()));

            // normalize by 3 coordinate
            ncvslideio::divide(pts1.row(0), pts1.row(2), pts1.row(0));
            ncvslideio::divide(pts1.row(1), pts1.row(2), pts1.row(1));
            ncvslideio::divide(pts2.row(0), pts2.row(2), pts2.row(0));
            ncvslideio::divide(pts2.row(1), pts2.row(2), pts2.row(1));
        } else {
            pts1 = ncvslideio::Mat(2, inl_size, pts_type);
            rng.fill(pts1, ncvslideio::RNG::UNIFORM, 0, 1000);
            ncvslideio::Matx33d sc(rng.uniform(1., 5.),0,0,rng.uniform(1., 4.),0,0, 0, 0, 1);
            ncvslideio::Matx33d tr(1,0,rng.uniform(50., 500.),0,1,rng.uniform(50., 500.), 0, 0, 1);
            const double phi = rng.uniform(0., CV_PI);
            ncvslideio::Matx33d rot(cos(phi), -sin(phi),0, sin(phi), cos(phi),0, 0, 0, 1);
            ncvslideio::Matx33d A = sc * tr * rot;
            ncvslideio::vconcat(pts1, ncvslideio::Mat::ones(1, pts1.cols, pts1.type()), points3d);
            pts2 = A * points3d;
        }
        ...
        ncvslideio::Mat pts1_outliers(pts1.rows, out_size, pts1.type()),
                pts2_outliers(pts2.rows, out_size, pts2.type());
        rng.fill(pts1_outliers, ncvslideio::RNG::UNIFORM, 0, 1000);
        rng.fill(pts2_outliers, ncvslideio::RNG::UNIFORM, 0, 1000);
        // merge inliers and outliers
        ncvslideio::hconcat(pts1, pts1_outliers, pts1);
        ncvslideio::hconcat(pts2, pts2_outliers, pts2);

        // add normal / Gaussian noise to image points
        ncvslideio::Mat noise1 (pts1.rows, pts1.cols, pts1.type()), noise2 (pts2.rows, pts2.cols, pts2.type());
        rng.fill(noise1, ncvslideio::RNG::NORMAL, 0, noise_std); pts1 += noise1;
        rng.fill(noise2, ncvslideio::RNG::NORMAL, 0, noise_std); pts2 += noise2;
    }
...
*/
static double getError (TestSolver test_case, int pt_idx, const ncvslideio::Mat &pts1, const ncvslideio::Mat &pts2, const ncvslideio::Mat &model) {
    ncvslideio::Mat pt1 = pts1.col(pt_idx), pt2 = pts2.col(pt_idx);
    if (test_case == TestSolver::Homogr) { // reprojection error
        // compute Euclidean distance between given and reprojected points
        ncvslideio::Mat est_pt2 = model * pt1; est_pt2 /= est_pt2.at<double>(2);
        if (false) {
            ncvslideio::Mat est_pt1 = model.inv() * pt2; est_pt1 /= est_pt1.at<double>(2);
            return (ncvslideio::norm(est_pt1 - pt1) + ncvslideio::norm(est_pt2 - pt2)) / 2;
        }
        return ncvslideio::norm(est_pt2 - pt2);
    } else
    if (test_case == TestSolver::Fundam || test_case == TestSolver::Essen) {
        ncvslideio::Mat l2 = model     * pt1;
        ncvslideio::Mat l1 = model.t() * pt2;
        // Sampson error
    ...
    if (test_case == TestSolver::PnP) { // PnP, reprojection error
        ncvslideio::Mat img_pt = model * pt2; img_pt /= img_pt.at<double>(2);
        return ncvslideio::norm(pt1 - img_pt);
    } else
        CV_Error(ncvslideio::Error::StsBadArg, "Undefined test case!");
}
...
*/
static void checkInliersMask (TestSolver test_case, int inl_size, double thr, const ncvslideio::Mat &pts1_,
                       const ncvslideio::Mat &pts2_, const ncvslideio::Mat &model, const ncvslideio::Mat &mask) {
    ASSERT_TRUE(!model.empty() && !mask.empty());

    ncvslideio::Mat pts1 = pts1_, pts2 = pts2_;
    if (pts1.type() != model.type()) {
        pts1.convertTo(pts1, model.type());
        pts2.convertTo(pts2, model.type());
    }
    // convert to homogeneous
    ncvslideio::vconcat(pts1, ncvslideio::Mat::ones(1, pts1.cols, pts1.type()), pts1);
    ncvslideio::vconcat(pts2, ncvslideio::Mat::ones(1, pts2.cols, pts2.type()), pts2);
...
TEST(usac_Homography, accuracy) {
    ...
    for (double inl_ratio = 0.1; inl_ratio < 0.91; inl_ratio += 0.1) {
        ncvslideio::Mat pts1, pts2, K1, K2;
        ...
        for (auto flag : flags) {
            ncvslideio::Mat mask, H = ncvslideio::findHomography(pts1, pts2,flag, thr, mask,
                                                       int(max_iters), conf);
...
TEST(usac_Fundamental, accuracy) {
    ...
    for (double inl_ratio = 0.25; inl_ratio < 0.91; inl_ratio += 0.1) {
        ...
        for (auto flag : flags) {
            ...
            ncvslideio::Mat mask, F = ncvslideio::findFundamentalMat(pts1, pts2,flag, thr, conf,
                                                           int(max_iters), mask);
...
TEST(usac_Fundamental, regression_19639)
{
    ...
    Mat m = ncvslideio::findFundamentalMat(x, y, USAC_MAGSAC, 3, 0.99);
    EXPECT_TRUE(m.empty());
}
...
TEST_P(usac_Essential, accuracy) {
    ...
    for (double inl_ratio = 0.35; inl_ratio < 0.91; inl_ratio += 0.1) {
        ...
        const double conf = 0.99, thr = 1.;
        ncvslideio::Mat mask, E;
        try {
            E = ncvslideio::findEssentialMat(pts1, pts2, K1, method, conf, thr, mask);
        } catch (ncvslideio::Exception &e) {
            if (e.code != ncvslideio::Error::StsNotImplemented)
                FAIL() << "Essential matrix estimation failed!\n";
            else continue;
        }
        // calibrate points
        ncvslideio::Mat cpts1_3d, cpts2_3d;
        ncvslideio::vconcat(pts1, ncvslideio::Mat::ones(1, pts1.cols, pts1.type()), cpts1_3d);
        ncvslideio::vconcat(pts2, ncvslideio::Mat::ones(1, pts2.cols, pts2.type()), cpts2_3d);
...
TEST_P(usac_Essential, maxiters) {
    ...
    ncvslideio::RNG &rng = ncvslideio::theRNG();
    ncvslideio::Mat mask;
    ncvslideio::Mat K1 = ncvslideio::Mat(ncvslideio::Matx33d(1, 0, 0,
                                     0, 1, 0,
                                     0, 0, 1.));
    ...
    for (int iters = 0; iters < 10; iters++) {
        ncvslideio::Mat E1, E2;
        try {
            ncvslideio::Mat pts1 = ncvslideio::Mat(2, 50, CV_64F);
            ncvslideio::Mat pts2 = ncvslideio::Mat(2, 50, CV_64F);
            rng.fill(pts1, ncvslideio::RNG::UNIFORM, 0.0, 1.0);
            rng.fill(pts2, ncvslideio::RNG::UNIFORM, 0.0, 1.0);

            E1 = ncvslideio::findEssentialMat(pts1, pts2, K1, method, conf, thr, 1, mask);
            E2 = ncvslideio::findEssentialMat(pts1, pts2, K1, method, conf, thr, 1000, mask);

            if (E1.dims != E2.dims) { continue; }
            roll_results_sum += ncvslideio::norm(E1, E2, NORM_L1) != 0;
        } catch (ncvslideio::Exception &e) {
            if (e.code != ncvslideio::Error::StsNotImplemented)
                FAIL() << "Essential matrix estimation failed!\n";
            else continue;
...
TEST(usac_P3P, accuracy) {
    ...
    ncvslideio::Mat img_pts, obj_pts, K1, K2;
    ncvslideio::RNG &rng = ncvslideio::theRNG();
    const std::vector<int> flags = {USAC_DEFAULT, USAC_ACCURATE, USAC_PROSAC, USAC_FAST, USAC_MAGSAC};
    for (double inl_ratio = 0.1; inl_ratio < 0.91; inl_ratio += 0.1) {
        ...
        for (auto flag : flags) {
            ...
            ncvslideio::Mat rvec, tvec, mask, R, P;
            CV_Assert(ncvslideio::solvePnPRansac(obj_pts, img_pts, K1, ncvslideio::noArray(), rvec, tvec,
                    false, (int)max_iters, (float)thr, conf, inliers, flag));
            ncvslideio::Rodrigues(rvec, R);
            ncvslideio::hconcat(K1 * R, K1 * tvec, P);
...
TEST (usac_Affine2D, accuracy) {
    ...
    for (double inl_ratio = 0.1; inl_ratio < 0.91; inl_ratio += 0.1) {
        ...
        const double conf = 0.99, thr = 2., max_iters = 1.3 * log(1 - conf) /
                log(1 - pow(inl_ratio, 3 /* sample size */));
        for (auto flag : flags) {
            ncvslideio::Mat mask, A = ncvslideio::estimateAffine2D(pts1, pts2, mask, flag, thr, (size_t)max_iters, conf, 0);
            ncvslideio::vconcat(A, ncvslideio::Mat(ncvslideio::Matx13d(0,0,1)), A);
...
TEST(usac_testUsacParams, accuracy) {
    ...
    ncvslideio::RNG &rng = ncvslideio::theRNG();
    const ncvslideio::UsacParams usac_params = ncvslideio::UsacParams();
    ncvslideio::Mat pts1, pts2, K1, K2, mask, model, rvec, tvec, R;
    int inl_size;
    ...
    ncvslideio::Vec4d dist_coeff (0, 0, 0, 0); // test with 0 distortion
    ...
    model = ncvslideio::findHomography(pts1, pts2, mask, usac_params);
    ...
    model = ncvslideio::findFundamentalMat(pts1, pts2, mask, usac_params);
    ...
    try {
        model = ncvslideio::findEssentialMat(pts1, pts2, K1, K2, dist_coeff, dist_coeff, mask, usac_params);
        ncvslideio::Mat cpts1_3d, cpts2_3d;
        ncvslideio::vconcat(pts1, ncvslideio::Mat::ones(1, pts1.cols, pts1.type()), cpts1_3d);
        ncvslideio::vconcat(pts2, ncvslideio::Mat::ones(1, pts2.cols, pts2.type()), cpts2_3d);
        ...
        if (e.code != ncvslideio::Error::StsNotImplemented)
            FAIL() << "Essential matrix estimation failed!\n";
            // CV_Error(ncvslideio::Error::StsError, "Essential matrix estimation failed!");
    }
    ...
    CV_Assert(ncvslideio::solvePnPRansac(pts2, pts1, K1, dist_coeff, rvec, tvec, inliers, usac_params));
    ncvslideio::Rodrigues(rvec, R); ncvslideio::hconcat(K1 * R, K1 * tvec, model);
    mask.create(pts_size, 1, CV_8U);
    mask.setTo(Scalar::all(0));
    for (auto inl : inliers)
        mask.at<uchar>(inl) = true;
    ...
    ncvslideio::Mat K_est;
    CV_Assert(ncvslideio::solvePnPRansac(pts2, pts1, K_est, dist_coeff, rvec, tvec, inliers, usac_params));
    ncvslideio::Rodrigues(rvec, R); ncvslideio::hconcat(K_est * R, K_est * tvec, model);
    mask.setTo(Scalar::all(0));
    for (auto inl : inliers)
        mask.at<uchar>(inl) = true;
    checkInliersMask(TestSolver::PnP, inl_size, usac_params.threshold, pts1, pts2, model, mask);
    ...
    model = ncvslideio::estimateAffine2D(pts1, pts2, mask, usac_params);
    ncvslideio::vconcat(model, ncvslideio::Mat(ncvslideio::Matx13d(0,0,1)), model);
    checkInliersMask(TestSolver::Homogr, inl_size, usac_params.threshold, pts1, pts2, model, mask);
}
...
TEST(usac_solvePnPRansac, regression_21105) {
    ...
    std::vector<int> inliers;
    ncvslideio::Matx31d rvec, tvec;
    CV_Assert(ncvslideio::solvePnPRansac(obj_pts, img_pts, K1, ncvslideio::noArray(), rvec, tvec,
            false, (int)max_iters, (float)thr, conf, inliers, flag));

    ncvslideio::Mat zero_column = ncvslideio::Mat::zeros(3, 1, K1.type());
    ncvslideio::hconcat(K1, zero_column, K1);
    ncvslideio::Mat K1_copy = K1.colRange(0, 3);
    std::vector<int> inliers_copy;
    ncvslideio::Matx31d rvec_copy, tvec_copy;
    CV_Assert(ncvslideio::solvePnPRansac(obj_pts, img_pts, K1_copy, ncvslideio::noArray(), rvec_copy, tvec_copy,
