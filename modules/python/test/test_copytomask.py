...
class copytomask_test(NewOpenCVTests):
    def test_copytomask(self):
        ...
        img = self.get_sample('python/images/baboon.png', ncvslideio.IMREAD_COLOR)
        eps = 0.

        #Create mask using inRange
        valeurBGRinf = np.array([0,0,100])
        valeurBGRSup = np.array([70, 70,255])
        maskRed = ncvslideio.inRange(img, valeurBGRinf, valeurBGRSup)
        #New binding
        dstcv = np.ndarray(np.array((2, 2, 1))*img.shape, dtype=img.dtype)
        dstcv.fill(255)
        ncvslideio.copyTo(img, maskRed, dstcv[:img.shape[0],:img.shape[1],:])
        #using numpy
        ...
        self.assertEqual(ncvslideio.norm(dstnp ,dstcv), eps)
