...
class get_cache_dir_test(NewOpenCVTests):
    def test_get_cache_dir(self):
        #New binding
        path = ncvslideio.utils.fs.getCacheDirectoryForDownloads()
    ...
    def get_cache_dir_imread_interop(self, ext):
        ...
        try:
            ncvslideio.imwrite(test_file_name, gold_image)
            read_from_file = ncvslideio.imread(test_file_name)
        finally:
            os.remove(test_file_name)

        self.assertEqual(ncvslideio.norm(gold_image, read_from_file), 0)
