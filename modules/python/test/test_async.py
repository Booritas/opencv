...
class AsyncTest(NewOpenCVTests):
    ...
    def test_async_simple(self):
        ...
        async_result = ncvslideio.utils.testAsyncArray(m)
        self.assertTrue(async_result.valid())
        ret, result = async_result.get(timeoutNs=10**6)  # 1ms
        self.assertTrue(ret)
        self.assertFalse(async_result.valid())
        self.assertEqual(ncvslideio.norm(m, result, ncvslideio.NORM_INF), 0)


    def test_async_exception(self):
        async_result = ncvslideio.utils.testAsyncException()
        self.assertTrue(async_result.valid())
        try:
            _ret, _result = async_result.get(timeoutNs=10**6)  # 1ms
            self.fail("Exception expected")
        except ncvslideio.error as e:
            self.assertEqual(ncvslideio.Error.StsOk, e.code)
