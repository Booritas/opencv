...
void initTests()
{
    ...
    const std::vector<ncvslideio::VideoCaptureAPIs> backends = ncvslideio::videoio_registry::getStreamBackends();
    const char* requireFFmpeg = getenv("OPENCV_TEST_VIDEOIO_BACKEND_REQUIRE_FFMPEG");
    if (requireFFmpeg && !isBackendAvailable(ncvslideio::CAP_FFMPEG, backends))
    {
