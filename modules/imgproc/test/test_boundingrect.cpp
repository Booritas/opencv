...
template <typename T>
ncvslideio::Rect calcBoundingRect(Mat pts)
{
...
TEST_P(Imgproc_BoundingRect_Types, accuracy)
{
    ...
    {
        SCOPED_TRACE(ncvslideio::format("k=%d", k));
        ...
        Rect result = ncvslideio::boundingRect(src);
...
TEST_P(Imgproc_BoundingRect_Types, alignment)
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        {
            SCOPED_TRACE(ncvslideio::format("i=%d, len=%d", i, len));
...
TEST(Imgproc_BoundingRect, bug_24217)
{
    for (int image_width = 3; image_width < 20; image_width++)
    {
        ...
        {
            ncvslideio::Rect rect(0, image_height - 1, 3, 1);

            ncvslideio::Mat image(ncvslideio::Size(image_width, image_height), CV_8UC1, ncvslideio::Scalar(0));
            image(rect) = 255;
