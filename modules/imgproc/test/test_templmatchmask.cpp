...
CV_ENUM(MatchTemplType, ncvslideio::TM_CCORR,  ncvslideio::TM_CCORR_NORMED,
                        ncvslideio::TM_SQDIFF, ncvslideio::TM_SQDIFF_NORMED,
                        ncvslideio::TM_CCOEFF, ncvslideio::TM_CCOEFF_NORMED)
...
{
    matchTemplate(img_testtype_, templ_testtype_, result_, ncvslideio::TM_SQDIFF, mask_testtype_);
...
{
    matchTemplate(img_testtype_, templ_testtype_, result_, ncvslideio::TM_SQDIFF_NORMED, mask_testtype_);
    // Naive implementation for one point
    Mat temp = img_roi_masked_ - templ_masked_;
    Scalar temp_s = sum(temp.mul(temp));
    double val = temp_s[0] + temp_s[1] + temp_s[2] + temp_s[3];
...
{
    matchTemplate(img_testtype_, templ_testtype_, result_, ncvslideio::TM_CCORR, mask_testtype_);
    // Naive implementation for one point
...
{
    matchTemplate(img_testtype_, templ_testtype_, result_, ncvslideio::TM_CCORR_NORMED, mask_testtype_);
    // Naive implementation for one point
    Scalar temp_s = sum(templ_masked_.mul(img_roi_masked_));
    double val = temp_s[0] + temp_s[1] + temp_s[2] + temp_s[3];
...
{
    matchTemplate(img_testtype_, templ_testtype_, result_, ncvslideio::TM_CCOEFF, mask_testtype_);
    // Naive implementation for one point
...
{
    matchTemplate(img_testtype_, templ_testtype_, result_, ncvslideio::TM_CCOEFF_NORMED, mask_testtype_);
    // Naive implementation for one point
    Scalar temp_s = sum(mask_);
    for (int i = 0; i < 4; i++)
    {
...
INSTANTIATE_TEST_CASE_P(SingleChannelMask, Imgproc_MatchTemplateWithMask2,
    Combine(
        Values(CV_32FC1, CV_32FC3, CV_8UC1, CV_8UC3),
        Values(CV_32FC1, CV_8UC1),
        Values(ncvslideio::TM_SQDIFF, ncvslideio::TM_SQDIFF_NORMED, ncvslideio::TM_CCORR, ncvslideio::TM_CCORR_NORMED,
               ncvslideio::TM_CCOEFF, ncvslideio::TM_CCOEFF_NORMED)));
