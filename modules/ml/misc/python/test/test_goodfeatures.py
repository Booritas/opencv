...
class TestGoodFeaturesToTrack_test(NewOpenCVTests):
    def test_goodFeaturesToTrack(self):
        ...
        results = dict([(t, ncvslideio.goodFeaturesToTrack(arr, numPoints, t, 2, useHarrisDetector=True)) for t in threshes])
        # Check that GoodFeaturesToTrack has not modified input image
        self.assertTrue(arr.tostring() == original.tostring())
        # Check for repeatability
        for i in range(1):
            results2 = dict([(t, ncvslideio.goodFeaturesToTrack(arr, numPoints, t, 2, useHarrisDetector=True)) for t in threshes])
            for t in threshes:
                self.assertTrue(len(results2[t]) == len(results[t]))
                for i in range(len(results[t])):
                    self.assertTrue(ncvslideio.norm(results[t][i][0] - results2[t][i][0]) == 0)
        ...
        for t0,t1 in zip(threshes, threshes[1:]):
            ...
            for i in range(len(r1)):
                self.assertTrue(ncvslideio.norm(r1[i][0] - r0[i][0])==0)
