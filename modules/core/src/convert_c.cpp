...
cvSplit( const void* srcarr, void* dstarr0, void* dstarr1, void* dstarr2, void* dstarr3 )
{
    ...
    for( i = 0; i < 4; i++ )
        nz += dptrs[i] != 0;
    CV_Assert( nz > 0 );
    std::vector<ncvslideio::Mat> dvec(nz);
    ...
    for( i = j = 0; i < 4; i++ )
    {
        if( dptrs[i] != 0 )
        {
            dvec[j] = ncvslideio::cvarrToMat(dptrs[i]);
    ...
    if( nz == src.channels() )
        ncvslideio::split( src, dvec );
    else
    {
        ncvslideio::mixChannels( &src, 1, &dvec[0], nz, &pairs[0], nz );
    }
}
         ...
         const void* srcarr3, void* dstarr )
{
    ...
    for( i = 0; i < 4; i++ )
        nz += sptrs[i] != 0;
    CV_Assert( nz > 0 );
    std::vector<ncvslideio::Mat> svec(nz);
    std::vector<int> pairs(nz*2);
    ...
    for( i = j = 0; i < 4; i++ )
    {
        if( sptrs[i] != 0 )
        {
            svec[j] = ncvslideio::cvarrToMat(sptrs[i]);
    ...
    if( nz == dst.channels() )
        ncvslideio::merge( svec, dst );
    else
    {
        ncvslideio::mixChannels( &svec[0], nz, &dst, 1, &pairs[0], nz );
    }
}
...
{
    ncvslideio::AutoBuffer<ncvslideio::Mat> buf(src_count + dst_count);

    int i;
    for( i = 0; i < src_count; i++ )
        buf[i] = ncvslideio::cvarrToMat(src[i]);
    for( i = 0; i < dst_count; i++ )
        buf[i+src_count] = ncvslideio::cvarrToMat(dst[i]);
    ncvslideio::mixChannels(&buf[0], src_count, &buf[src_count], dst_count, from_to, pair_count);
}
                   ...
                   double scale, double shift )
{
    ...
    ncvslideio::convertScaleAbs( src, dst, scale, shift );
}
...
{
    ncvslideio::Mat src = ncvslideio::cvarrToMat(srcarr), dst = ncvslideio::cvarrToMat(dstarr), lut = ncvslideio::cvarrToMat(lutarr);
                          ...
                          double a, double b, int norm_type, const CvArr* maskarr )
{
    ...
    ncvslideio::normalize( src, dst, a, b, norm_type, dst.type(), mask );
}
