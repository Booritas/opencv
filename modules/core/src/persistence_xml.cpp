...
public:
    ...
    void writeTag( const char* key, int tag_type, const std::vector<std::string>& attrlist=std::vector<std::string>() )
    {
        ...
        if( tag_type == CV_XML_OPENING_TAG || tag_type == CV_XML_EMPTY_TAG )
        {
            if( FileNode::isCollection(struct_flags) )
            {
                if( FileNode::isMap(struct_flags) ^ (key != 0) )
                    CV_Error( ncvslideio::Error::StsBadArg, "An attempt to add element without a key to a map, "
        ...
        if( !key )
            ...
            CV_Error( ncvslideio::Error::StsBadArg, "A single _ is a reserved tag name" );
        ...
        *ptr++ = '<';
        ...
        {
            if( !attrlist.empty() )
                CV_Error( ncvslideio::Error::StsBadArg, "Closing tag should not include any attributes" );
            *ptr++ = '/';
        }

        if( !cv_isalpha(key[0]) && key[0] != '_' )
            CV_Error( ncvslideio::Error::StsBadArg, "Key should start with a letter or _" );
        ...
        for( i = 0; i < len; i++ )
        {
            ...
            if( !cv_isalnum(c) && c != '_' && c != '-' )
                CV_Error( ncvslideio::Error::StsBadArg, "Key name may only contain alphanumeric characters [a-zA-Z0-9], '-' and '_'" );
            ptr[i] = c;
        }
        ptr += len;
    ...
    void write(const char* key, const char* str, bool quote)
    {
        ...
        if( !str )
            CV_Error( ncvslideio::Error::StsNullPtr, "Null string pointer" );

        len = (int)strlen(str);
        if( len > CV_FS_MAX_LEN )
            CV_Error( ncvslideio::Error::StsBadArg, "The written string is too long" );
    ...
    void writeScalar(const char* key, const char* data)
    {
        ...
        {
            CV_Error( ncvslideio::Error::StsError, "At present, output Base64 data only." );
        }
        ...
        else
        {
            ...
            if( key )
                CV_Error( ncvslideio::Error::StsBadArg, "elements with keys can not be written to sequence" );
    ...
    void writeComment(const char* comment, bool eol_comment)
    {
        ...
        if( !comment )
            CV_Error( ncvslideio::Error::StsNullPtr, "Null comment" );

        if( strstr(comment, "--") != 0 )
            CV_Error( ncvslideio::Error::StsBadArg, "Double hyphen \'--\' is not allowed in the comments" );
