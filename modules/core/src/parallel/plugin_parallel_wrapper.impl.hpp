...
public:
    ...
    PluginParallelBackend(const std::shared_ptr<ncvslideio::plugin::impl::DynamicLib>& lib)
        : lib_(lib)
        , plugin_api_(NULL)
    {
        initPluginAPI();
    ...
    std::shared_ptr<ncvslideio::parallel::ParallelForAPI> create() const
    {
        ...
        if (plugin_api_->v0.getInstance)
        {
            if (CV_ERROR_OK == plugin_api_->v0.getInstance(&instancePtr))
            {
                ...
                return std::shared_ptr<ncvslideio::parallel::ParallelForAPI>(instancePtr, [](ncvslideio::parallel::ParallelForAPI*){});  // empty deleter
            }
        }
        return std::shared_ptr<ncvslideio::parallel::ParallelForAPI>();
    }
};
...
public:
    ...
    std::shared_ptr<ncvslideio::parallel::ParallelForAPI> create() const CV_OVERRIDE
    {
        if (!initialized)
        {
