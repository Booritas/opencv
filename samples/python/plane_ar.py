...
class App:
    def __init__(self, src):
        ...
        ncvslideio.createTrackbar('focal', 'plane', 25, 50, common.nothing)
    ...
    def draw_overlay(self, vis, tracked):
        ...
        fx = 0.5 + ncvslideio.getTrackbarPos('focal', 'plane') / 50.0
        h, w = vis.shape[:2]
        K = np.float64([[fx*w, 0, 0.5*(w-1)],
                        [0, fx*w, 0.5*(h-1)],
                        [0.0,0.0,      1.0]])
        dist_coef = np.zeros(4)
        _ret, rvec, tvec = ncvslideio.solvePnP(quad_3d, tracked.quad, K, dist_coef)
        verts = ar_verts * [(x1-x0), (y1-y0), -(x1-x0)*0.3] + (x0, y0, 0)
        verts = ncvslideio.projectPoints(verts, rvec, tvec, K, dist_coef)[0].reshape(-1, 2)
        for i, j in ar_edges:
            (x0, y0), (x1, y1) = verts[i], verts[j]
            ncvslideio.line(vis, (int(x0), int(y0)), (int(x1), int(y1)), (255, 255, 0), 2)
