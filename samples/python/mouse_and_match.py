...
class App():
    ...
    def onmouse(self, event, x, y, flags, param):
        if event == ncvslideio.EVENT_LBUTTONDOWN:
            ...
            if self.sel[2] > self.sel[0] and self.sel[3] > self.sel[1]:
                ...
                result = ncvslideio.matchTemplate(self.gray, patch, ncvslideio.TM_CCOEFF_NORMED)
                result = np.abs(result)**3
                _val, result = ncvslideio.threshold(result, 0.01, 0, ncvslideio.THRESH_TOZERO)
                result8 = ncvslideio.normalize(result, None, 0, 255, ncvslideio.NORM_MINMAX, ncvslideio.CV_8U)
                ncvslideio.imshow("result", result8)
            ...
            if flags & ncvslideio.EVENT_FLAG_LBUTTON:
                ...
                img = ncvslideio.cvtColor(self.gray, ncvslideio.COLOR_GRAY2BGR)
                ncvslideio.rectangle(img, (self.sel[0], self.sel[1]), (self.sel[2], self.sel[3]), (0,255,255), 1)
                ncvslideio.imshow("gray", img)
            else:
    ...
    def run(self):
        ...
        ncvslideio.namedWindow("gray",1)
        ncvslideio.setMouseCallback("gray", self.onmouse)
        ...
        for infile in glob.glob( os.path.join(path, '*.*') ):
            ...
            if ext == "png" or ext == "jpg" or ext == "bmp" or ext == "tiff" or ext == "pbm":
                ...
                img = ncvslideio.imread(infile, ncvslideio.IMREAD_COLOR)
                if img is None:
                    continue
                self.sel = (0,0,0,0)
                self.drag_start = None
                self.gray = ncvslideio.cvtColor(img, ncvslideio.COLOR_BGR2GRAY)
                ncvslideio.imshow("gray", self.gray)
                if ncvslideio.waitKey() == 27:
                    break
