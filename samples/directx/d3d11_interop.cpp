...
{
public:
    D3D11WinApp(int width, int height, std::string& window_name, ncvslideio::VideoCapture& cap)
    ...
    int create(void)
    {
        ...
        {
            m_oclCtx = ncvslideio::directx::ocl::initializeContextFromD3D11Device(m_pD3D11Dev);
        }
    ...
    int get_surface(ID3D11Texture2D** ppSurface, bool use_nv12)
    {
        ...
        if (use_nv12)
        {
            ncvslideio::cvtColor(m_frame_bgr, m_frame_i420, ncvslideio::COLOR_BGR2YUV_I420);
        ...
        else
        {
            ...
            ncvslideio::Mat m(m_height, m_width, CV_8UC4, mappedTex.pData, mappedTex.RowPitch);
    ...
    int render()
    {
        try
        {
            ...
            case MODE_GPU_NV12:
            {
                ...
                ncvslideio::directx::convertFromD3D11Texture2D(pSurface, u);
                ...
                ncvslideio::String strMode = ncvslideio::format("mode: %s", m_modeStr[mode].c_str());
                ncvslideio::String strProcessing = m_demo_processing ? "blur frame" : "copy frame";
                ncvslideio::String strTime = ncvslideio::format("time: %4.3f msec", m_timer.getTimeMilli());
                ncvslideio::String strDevName = ncvslideio::format("OpenCL device: %s", m_oclDevName.c_str());
                ...
                ncvslideio::directx::convertToD3D11Texture2D(u, pSurface);
                ...
                if (mode == MODE_GPU_NV12)
                {
                    ...
                    // process video frame on CPU
                    {
                        ...
                        ncvslideio::Mat frame_nv12(m_height + (m_height / 2), m_width, CV_8UC1, mappedTex.pData, mappedTex.RowPitch);
                        ncvslideio::cvtColor(frame_nv12, m_frame_rgba, ncvslideio::COLOR_YUV2RGBA_NV12);
...
protected:
    void convert_I420_to_NV12(ncvslideio::Mat& i420, ncvslideio::Mat& nv12, int width, int height)
    {
...
private:
    ...
    ncvslideio::Mat                 m_frame_i420;
    ncvslideio::Mat                 m_frame_nv12;
};
