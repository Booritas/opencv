...
template<typename T> struct step_functor : public thrust::unary_function<int, int>
{
    ...
    __host__ step_functor(ncvslideio::cuda::GpuMat& mat)
    {
        CV_Assert(mat.depth() == ncvslideio::DataType<T>::depth);
        columns = mat.cols;
...
*/
template<typename T>
thrust::permutation_iterator<thrust::device_ptr<T>, thrust::transform_iterator<step_functor<T>, thrust::counting_iterator<int>>>  GpuMatBeginItr(ncvslideio::cuda::GpuMat mat, int channel = 0)
{
    if (channel == -1)
    {
        mat = mat.reshape(1);
...
*/
template<typename T>
thrust::permutation_iterator<thrust::device_ptr<T>, thrust::transform_iterator<step_functor<T>, thrust::counting_iterator<int>>>  GpuMatEndItr(ncvslideio::cuda::GpuMat mat, int channel = 0)
{
    if (channel == -1)
    {
        mat = mat.reshape(1);
