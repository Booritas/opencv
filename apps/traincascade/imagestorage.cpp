...
bool CvCascadeImageReader::PosReader::create( const string _filename )
{
    ...
    if( fread( &count, sizeof( count ), 1, file ) != 1 ||
        ...
        fread( &tmp, sizeof( tmp ), 1, file ) != 1 )
        CV_Error_( ncvslideio::Error::StsParseError, ("wrong file format for %s\n", _filename.c_str()) );
...
bool CvCascadeImageReader::PosReader::get( Mat &_img )
{
    ...
    if( elements_read != 1 )
        CV_Error( ncvslideio::Error::StsBadArg, "Can not get new positive sample. The most possible reason is "
    ...
    if( elements_read != (size_t)(vecSize) )
        CV_Error( ncvslideio::Error::StsBadArg, "Can not get new positive sample. Seems that vec-file has incorrect structure.\n");

    if( feof( file ) || last++ >= count )
        CV_Error( ncvslideio::Error::StsBadArg, "Can not get new positive sample. vec-file is over.\n");
